...
def initialize_bot(bot, bot_id):
    ...
    def start_attack_reply(message, target, port, time, owner_name):
        ...
        for i in range(num_instances):
            full_command = ['nohup', './neon', str(target), str(port), str(time), str(threads_per_instance)]
        ...
        markup.add(InlineKeyboardButton("HELLO", callback_data="hello" + str(chat_id)))
    ...
    @bot.callback_query_handler(func=lambda call: call.data.startswith("@NEONxCHEATZ2"))
